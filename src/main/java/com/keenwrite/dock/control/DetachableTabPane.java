...
public class DetachableTabPane extends TabPane {
  ...
  /**
   * Reference to the source of drag event. We can't use {@code event
   * .getGestureSource()} because it is null when the target on a different
   * stage.
   */
  ...
  private static final DetachableTabPathModel pathModel =
      new DetachableTabPathModel( path );
  ...
  private void initDropButton() {
    ...
    final GridPane posGrid = new GridPane();
  ...
  private void attachListeners() {
    ...
    sceneProperty().addListener(
        ( ObservableValue<? extends Scene> ov, Scene t, Scene t1 ) -> {
          if( t == null && t1 != null ) {
            if( getScene().getWindow() != null ) {
              ...
              getScene()
                  .windowProperty()
                  .addListener(
                      ( ObservableValue<? extends Window> ov1,
                        Window t2, Window t3 ) -> {

                        if( t2 == null && t3 != null ) {
                          t3.addEventHandler( WindowEvent.WINDOW_SHOWN,
                                              ( t4 ) -> initiateDragGesture(
                                                  true ) );
                        }
                      } );
            }
    ...
    addEventHandler( DragEvent.ANY, ( DragEvent event ) -> {
      try {
        if( DRAG_SOURCE == null ) {
          return;
        }
        if( event.getEventType() == DragEvent.DRAG_OVER ) {
          if( DetachableTabPane.this.scope.get()
                                          .equals( DRAG_SOURCE.getScope() ) ) {
          ...
          if( DRAG_SOURCE != DetachableTabPane.this ) {
            ...
            Platform.runLater( () -> DetachableTabPane.this.getSelectionModel()
                                                           .select( selectedtab ) );
            event.setDropCompleted( true );
          }
          else {
  ...
  private SplitPane findParentSplitPane( Node control ) {
    ...
    Set<Node> lstSplitpane = control.getScene()
                                    .getRoot()
                                    .lookupAll( ".split-pane" );
  ...
  private void adjacent() {
    ...
    if( targetSplitPane != null && requestedOrientation == targetSplitPane.getOrientation() ) {
      requestedIndex = targetSplitPane.getItems()
                                      .indexOf( DetachableTabPane.this );
    }
    ...
    if( targetSplitPane == null ) {
      ...
      if( targetSplitPane.getOrientation() == requestedOrientation ) {
        ...
        int indexTabPane = targetSplitPane.getItems()
                                          .indexOf( DetachableTabPane.this );
  ...
  private void repaintPath( DragEvent event, int source ) {
    ...
    if( hasTab && btnLeft.contains( btnLeft.screenToLocal( event.getScreenX(),
      ...
      pathModel.refresh( 0,
                         0,
                         DetachableTabPane.this.getWidth() / 2,
                         DetachableTabPane.this.getHeight() );
      pos = Pos.CENTER_LEFT;
    }
    else if( hasTab && btnRight.contains( btnRight.screenToLocal( event.getScreenX(),
                                                                  event.getScreenY() ) ) ) {
      double pathWidth = DetachableTabPane.this.getWidth() / 2;
      pathModel.refresh( pathWidth,
                         0,
                         pathWidth,
                         DetachableTabPane.this.getHeight() );
      pos = Pos.CENTER_RIGHT;
    }
    else if( hasTab && btnTop.contains( btnTop.screenToLocal( event.getScreenX(),
                                                              event.getScreenY() ) ) ) {
      pathModel.refresh( 0, 0, getWidth(), getHeight() / 2 );
      pos = Pos.TOP_CENTER;
    }
    else if( hasTab && btnBottom.contains( btnBottom.screenToLocal( event.getScreenX(),
                                                                    event.getScreenY() ) ) ) {
      ...
      pathModel.refresh( tabpos,
                         DetachableTabPane.this.getWidth(),
                         DetachableTabPane.this.getHeight() );
    }
  }
  ...
  private void addGesture( final TabPane tabPane, final Node node ) {
    ...
    node.setOnDragDone( ( DragEvent event ) -> {
      ...
      if( DRAG_SOURCE.getScene() != null && DRAG_SOURCE.getScene()
                                                       .getWindow() instanceof TabStage ) {
  ...
  private void closeStageIfNeeded( TabStage stage ) {
    ...
    if( empty ) {
      //there is a case where lookup .tab-pane style doesn't return all 
      // TabPane. So we need to lookup by SplitPane and scan through it
      Set<Node> setSplitpane = stage.getScene()
                                    .getRoot()
                                    .lookupAll( ".split-pane" );
  ...
  private void removeFromParent( DetachableTabPane tabPaneToRemove ) {
    ...
    if( !tabPaneToRemove.isCloseIfEmpty() ) {
      ...
      final List<Tab> lstTab = new ArrayList<>( sibling.getTabs() );
  ...
  private DetachableTabPane findSibling( SplitPane sp,
                                         DetachableTabPane tabPaneToRemove ) {
    for( Node sibling : sp.getItems() ) {
      if( tabPaneToRemove != sibling
          && sibling instanceof DetachableTabPane
          && tabPaneToRemove.getScope()
                            .equals( ((DetachableTabPane) sibling).getScope() ) ) {
   ...
   */
  public void setSceneFactory(
      Callback<DetachableTabPane, Scene> sceneFactory ) {
   ...
   */
  public void setStageOwnerFactory(
      Callback<Stage, Window> stageOwnerFactory ) {
   ...
   * {@link TabClosingPolicy}.
   */
  public void setDetachableTabPaneFactory(
      DetachableTabPaneFactory detachableTabPaneFactory ) {
    if( detachableTabPaneFactory == null ) {
      throw new IllegalArgumentException( "detachableTabPaneFactory cannot " +
                                              "null" );
    }
  ...
  private Callback<Stage, Window> stageOwnerFactory = p -> {
    final var scene = DetachableTabPane.this.getScene();
  ...
  private class TabStage extends Stage {
    ...
    public TabStage( final Tab tab ) {
      ...
      scene.getStylesheets()
           .addAll( DetachableTabPane.this.getScene().getStylesheets() );
      setScene( scene );
